...
Every key `agent.toml` accepts, derived from the config structs. Edit the doc comment on the field, not this table. Acceptance of a key is not a Stable-support claim; consult the [capability matrix](CAPABILITY_MATRIX.md) before enabling Preview surfaces.
