...
| Sandboxed subprocess tools | Preview | Shell is enabled in example | Linux Landlock; macOS Seatbelt | `[isolation]`, compatible worker/backend | Registry invocation is fail-closed and protocol-checked; actual-process MCP isolation and required platform qualification remain incomplete | No-body-invocation refusal tests pass; promotion still requires real Landlock/Seatbelt required jobs and `MCP-001` |
