...
| Append-only session log, projection, and fork | Stable within one typed principal | Enabled | Portable | Session backend | Principal isolation is enforced; legacy-row migration and `/clear` epoch advancement remain open | Projection/fork and cross-agent/channel/sender isolation tests; migration and reset promotion require `ID-002` and `STATE-001` |
...
| Shared or multi-principal memory | Preview | Shared writes disabled | Portable | `[memory.policy]`, `[[memory.shared_domains]]` | Private memory is principal-isolated; shared write authority, legacy migration, and retention remain incomplete | Principal isolation regression; promotion requires `ID-002` migration tests and `MEM-001` behavior tests |
