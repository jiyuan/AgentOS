...
| Subagent delegation and sub-orchestrator templates | Preview | Reachable from example routing | Portable, subject to tool backends | Routing, subagent/template files, child policies | Current narrowing can widen `AskUser` or constrained authority; identity is incomplete | Promotion requires policy lattice property tests in `AUTH-002` and principal work in `ID-001` |
