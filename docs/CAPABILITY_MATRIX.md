...
| Release archive and installed wrapper | Preview | Self-contained archive; wrapper available | Linux/macOS targets | Installer prefix and packaged workspace | Archive workspace/docs and direct offline echo are checked; installed layout, wrapper diagnostics, and pathless resume still disagree with docs | `REL-001` archive check is implemented; promotion requires the `REL-002` clean-room install/wrapper smoke |
