...
| Concrete ticketed approval | Stable with principal-bound resolvers | Enabled as policy requires | Portable | `[policy]`, `[approval]`, remote administrator ids | Initiator/same-chat administrator binding is enforced; durable decision history is not yet Stable | Pause/resume and cross-participant resolver tests; broader promotion requires `AUD-001` |
...
| Telegram and Feishu production channels | Preview | Disabled | Portable plus remote service | `[channels.telegram]` or `[channels.feishu]`, credentials, explicit sender/chat policy | Authenticated sender allowlists and principal-bound approvals are enforced; durable ACK/replay and stable buffered output remain incomplete | Promotion requires `ING-001`, `STATE-001`, and `GW-001` plus live fixtures |
...
| Subagent delegation and sub-orchestrator templates | Preview | Reachable from example routing | Portable, subject to tool backends | Routing, subagent/template files, child policies | Strict per-call narrowing, principal-keyed child sessions/forks, and principal-approved exact, expiring, non-transitive delegation grants are enforced; complete safety-event retention remains part of `AUD-001` | Strict policy-lattice, grant scope/expiry/persistence, and principal-keyed fork tests; promotion also depends on the remaining Preview tool backends |
