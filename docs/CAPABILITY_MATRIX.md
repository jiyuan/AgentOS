...
| Append-only session log, projection, and fork | Stable within one principal | Enabled | Portable | Session backend | `/clear` epoch semantics and typed cross-channel identity remain open | Projection/fork integration and golden tests; multi-principal promotion requires `ID-001` and `STATE-001` |
...
| Shared or multi-principal memory | Preview | Shared writes disabled | Portable | `[memory.policy]`, `[[memory.shared_domains]]` | Principal isolation, write authority, and retention are incomplete | Promotion requires `ID-001`, migration tests, and `MEM-001` behavior tests |
...
| TUI and one-shot local operation | Stable from source | TUI enabled | Portable | `[channels.tui]` and provider credentials, or offline fixture | Packaged-install self-containment is a separate Preview capability | Core/CLI tests; artifact use requires `REL-001` and `REL-002` |
| Release archive and installed wrapper | Preview | Packaging scripts available | Linux/macOS targets | Installer prefix and packaged workspace | Archive is not yet self-contained and wrapper/docs commands disagree | Promotion requires clean-room archive install and offline echo smoke tests in M1 |
...
| Background jobs | Stable for one trusted channel/conversation namespace | Tools enabled | Portable | `[jobs]`, `job_*` tools | Ownership is not yet based on the complete typed principal; descendant cleanup is incomplete | Job integration tests; cross-channel promotion requires `ID-001` and `PROC-001` |
...
| Sandboxed subprocess tools | Preview | Shell is enabled in example | Linux Landlock; macOS Seatbelt | `[isolation]`, compatible worker/backend | Current fallback and executor compatibility do not meet the fail-closed contract | Promotion requires `SBX-001`, real platform enforcement, and no-body-invocation refusal tests |
...
| Subagent delegation and sub-orchestrator templates | Preview | Reachable from example routing | Portable, subject to tool backends | Routing, subagent/template files, child policies | Strict per-call narrowing is enforced; explicit delegation grants and principal identity are incomplete | Strict policy-lattice property tests landed in `AUTH-002`; promotion still requires the explicit grant model and `ID-001` |
