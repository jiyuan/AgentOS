...
| Ticketed approval | Stable | all | `[policy]` | Every prompt has a unique approval instance backed by a once-generated 128-bit OS-CSPRNG nonce and checked counter. Resolution requires an opaque witness bound to the exact pending ticket and actor; administrator entries are complete agent/channel/conversation/sender principals. | unit, integration |
| Policy engine (`allow` / `deny` / `ask_user`) | Stable | all | `[policy]` | — | unit, integration |
| Sub-agent policy narrowing | Stable | all | `[[subagents]]` | Exact over actions *and* arguments, decided by witness calls rather than by comparing rules. An elevation needs a lifetime-bounded `[[subagents.delegation_grants]]` template; each runtime grant is bound to the initiating actor, delegatee, and generation, expires within one hour, is non-transitive, and shares a stable ID across issuance/use events ([ADR-0001](adr/0001-POLICY_NARROWING.md), `AUTH-002`, `AUTH-004`) | unit, integration, property |
...
| Durable safety events (`safety_events`) | Stable | all | a SQLite session/memory store | Thirteen event kinds, append-only, principal-stamped. Tool arguments enter only as a SHA-256 digest and reasons are capped at 512 bytes. A failed required append is logged as `safety_log_write_failed` and fails the affected run closed with `RunError::SafetyEvidence`; an entrypoint with no configured store remains explicitly detached ([ADR-0005](adr/0005-SAFETY_EVENTS.md), `AUD-002`) | unit, integration, failpoint |
...
| Append-only session log with projection | Stable | all | — | `/clear` appends an epoch marker and deletes nothing. The transcript is keyed by conversation and the epoch by participant, so a group member clears their own view and a senderless clear moves everybody — two participants can therefore see different prefixes of one log. Deletion is `agentos-gateway purge` ([ADR-0006](adr/0006-CLEAR_EPOCH.md)) | integration, golden |
| Principal-keyed sessions and jobs | Stable | all | — | `Session` and `JobRegistry` key on `Principal`, so `telegram:42`, `feishu:42` and a second agent's `telegram:42` are separate. A database written before this is refused at startup until `agentos-gateway migrate` rebuilds it; task workspaces are deliberately still keyed by task id (see §13 of the remediation plan) | unit, integration |
