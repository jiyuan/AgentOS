...
│   │   ├── retention/        # Age and byte ceilings for the stores that grow.
...
- **Run loop is a typed state machine, and the ordering is enforced.** `RunLoopState` is an enum (`Start`, `Plan`, `Approve`, `Act`, `Observe`, `Paused`, `Finish`). `step()` consumes `self`, returns the next state or a `StepFailure` carrying the state back out, and its `match` is exhaustive. `ActCtx` carries a private `Authorization` with no public constructor, so a hand-built `ActCtx` does not compile; the witness names the plan it was issued for, so a caller holding a real one cannot swap the plan; and `act()` re-decides against the live policy, accepting `ask_user` only when a human answered. A new edge goes in `tests/loop_transitions.rs` before it goes in the loop.
- **A config key does something, or the load fails.** Every deserializable struct in `crates/agentos-core/src/config/` carries `deny_unknown_fields`, so a typo is a load-time error rather than a silent default. Every key has a doc comment — `config/undocumented.txt` is empty and ratcheted both ways. A key that no longer decides anything is `Option`-shaped, warns at load, and is listed in `config/maturity.txt` as `deprecated`; a `preview` key is off by default. `agent.id` reaches `AgentRuntime::active_agent` and therefore every trace, episode, memory record, and safety event. `agentos-gateway config` is derived from the same walk that generates the catalogs, so a key in the structs cannot be missing from it — do not hand-write that output.
- **`[memory.policy]` is memory's authority.** Read, write, forget, and shared writes are decided by `[memory.policy]` and nothing else; `runtime::tools_config` emits the `memory` rules from it rather than hardcoding them, and a `[policy] allowlist` entry naming `memory` fails the load rather than quietly winning. A shared-domain write needs three permissions — the global `shared_writes`, the domain's own `write`, and a caller holding it — intersected before `authorize_scope` sees them.
- **Guardrails ≠ Approve.** Guardrails check content (input/tool/output). Approve checks permission (allow/deny/ask_user). Both are required at their designated points in the loop. Every terminal reply crosses the output policy, the loop's own cancellation and budget notices included — `loop/output.rs` is the single gate, and there is no path that decides for itself that it is exempt. Streaming (`[channels] provisional_streaming`, off by default) is the only way to emit bytes ahead of the check, because a chunk forwarded during `plan()` cannot be retracted.
- **A safety decision is an event, never an absence.** Approval requested and resolved, policy denial, guardrail trip, sandbox refusal, cancellation, delegation-grant issuance and use, and terminal error all append a row to `safety_events` through `crates/agentos-core/src/audit/`. Deletion is never the record of an outcome: a resolved interruption is marked `consumed` and retained, not removed. Events carry a `Principal`, a subject, and a bounded reason; a tool's arguments enter only as `ArgumentDigest`, and `SafetyEvent` has no field that would accept them. `SafetyLog` lives in the core, not in `agentos-interfaces`, for the same reason `approve` does. `RunLoopState::step` returns `StepFailure`, which carries the state back out, so a run that ends badly still persists its trace — add a new failure path only if it hands the state back too. Nothing deletes from `safety_events` or `memory_access_log` on a timer: their only retention is `agentos-gateway purge --audit --before DATE`, which reports first, applies only against the count typed back, and writes an `audit_purged` row into the store it just shortened.
...
- **Every store that grows has a named bound, and the record is not swept.** `crates/agentos-core/src/retention/` applies `[retention]`, `[spill]` and `[jobs].completed_retention_secs` to the six stores that are by-products — traces, attachments, the gateway log, spill, settled ingress rows, finished jobs — from the gateway's idle phase under a `gateway.retention` lease. Each file store takes an age ceiling *and* a byte quota, because a busy afternoon outweighs a quiet month. The session log and the two audit stores are never swept; what bounds them is `agentos-gateway purge`, which reports before it applies. A new durable store needs a ceiling in the same pass that adds it, and the sweep is driven from a private function in the gateway binary on purpose: unwiring it fails the build, which is what the never-called `AgentRuntime::sweep_spill` did not.
- **All channels are bounded.** Every `tokio::mpsc` channel must use `channel(cap)`, never `unbounded_channel()`.
- **An accepted message is recorded before it is run, and settled after.** `gateway/ingress.rs` keys on `(channel_id, transport event id)` and answers fresh / retry / suppress; the row is written before the envelope reaches a shard and settled once the turn ends, so the crash window falls inside *retry* rather than between the two. A channel publishes its transport's own id under `INGRESS_ID_KEY` — never one the gateway invented, because a content hash merges two identical messages and a timestamped hash calls every redelivery new. A resumable cursor is persisted only after the event at that position is in the ledger.
- **Who is serving is a held lock, not a number in a file.** `gateway/control.rs` holds `flock(LOCK_EX)` on the control file for the life of the serving process, so *locked* means that process is alive and *unlocked* means nobody is, with no `kill -0` liveness guess and no stale-file heuristic. This is the one state file not written through `paths::write_private_atomic`: `rename` would drop the lock. `SIGTERM` sets a flag, the router stops accepting, and the drain is bounded by `[gateway] shutdown_grace_secs` — past which the gateway reports what it abandoned rather than hanging.
- **MCP is MCP.** `tools/mcp/protocol.rs` is JSON-RPC 2.0 and the pinned MCP revision: `initialize` before anything else, capability negotiation, `tools/list` walked to the end of its `nextCursor` chain, content blocks, `notifications/cancelled` for a call this side gave up on, and structured errors both ways. A reply is matched to its request **by id** — never by arrival order, which is what let one stray line shift every later answer onto the wrong question. Every bound is named: frame bytes, in-flight requests, stderr kept, pages walked, tools per server, restarts per window. Interoperability is tested against a fixture that imports nothing from this repository (`tests/fixtures/mcp_server.py`); a fixture built from these types would only prove the two halves of one dialect agree.
...
- **A session is keyed by a principal, not by a conversation id.** `Session::{load, append, fork}` take `&Principal` and read two names out of it: `conversation_name()` identifies the *transcript*, which everyone in a conversation shares, and `storage_name()` identifies the *participant*, which only the `/clear` epoch uses. `session_epochs` carries both columns and `current_epoch` takes the maximum, so a group member clears their own view and a senderless clear moves everybody. The same rule holds for `JobRegistry`, whose conversation fence decides who may read a job's output. A database still keyed by bare conversation ids is refused at startup by `agentos-gateway start` and `serve` rather than read as empty — `agentos-gateway migrate` rebuilds it.
- **The session log is append-only, without qualification.** Compaction, elision, and fork are all *projections* over the log (`prompt/projection.rs`), never mutations of it, and so is `/clear`: it writes a row to `session_epochs` and `Session::load` returns only items at or after the newest epoch. Every reader of `session_items` must respect the epoch, which is why the filter lives beside the two queries in `memory/sqlite.rs` and not at call sites — one that forgets it resurrects history. The single deletion is `SqliteStore::purge_session`, reachable only from `agentos-gateway purge` with the conversation named twice, and it emits a `session_purged` safety event. `purge --sessions --before DATE` removes whole idle conversations *through that same method*, one call and one event each — it does not add a statement of its own, and it does not trim old items off a live conversation, which would leave a history beginning mid-sentence. Do not add a second path that deletes.
...
- **A spill locator is not a path.** Oversized tool output is persisted behind `spill:<run>/<artifact>`, which names a place in the configured store and carries nothing about the host into the transcript. `spill_read` is the only retrieval path: it resolves through `paths::RootDir` against the store, and refuses a locator the calling run's own transcript does not cite. Do not hand the model a filesystem path for anything the runtime wrote.
