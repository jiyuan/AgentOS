...
- **Sub-agent policies only narrow.** `Policy::narrow(&parent, &child)` returns `Err` when the child reaches an action the parent does not hold, and every tool call in a sub-agent re-enters the loop at the Approve state — no shortcuts, including for MCP-originated calls. Narrowing is currently enforced over *which tools*, not *under what conditions*: `parent_exposes_tool` (`approve/mod.rs`) matches on tool name alone, so a child rule naming a tool the parent gates behind `AskUser` or argument constraints is accepted as a blanket `Allow`. Treat the stated invariant as the target — do not add code that leans on the gap. Closing it is `AUTH-002` in `docs/AUDIT_REMEDIATION_PLAN.md`.
