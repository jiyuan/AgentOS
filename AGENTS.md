...
- **A config key does something, or the load fails.** Every deserializable struct in `crates/agentos-core/src/config/` carries `deny_unknown_fields`, so a typo is a load-time error rather than a silent default. A key that no longer decides anything is `Option`-shaped, warned about at load, and named as deprecated in its doc comment — it is never left in place doing nothing. `agent.id` reaches `AgentRuntime::active_agent` and therefore every trace, episode, memory record, and safety event.
